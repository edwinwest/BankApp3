...
//solve coverity serialzable defect
import java.io.Serializable;

@WebServlet(STAFF_DASHBOARD_PAGE)
public class StaffDashboardServlet extends DefaultServlet implements Serializable{
	...
	private transient ClientInfoDAO clientInfoDAO = new ClientInfoDAOImpl();
	...
	private transient TransactionCodesDAO transactionCodesDAO = new TransactionCodesDAOImpl();
	private transient ClientTransactionDAO clientTransactionDAO = new ClientTransactionDAOImpl();
	...
	private void onTransactionDecisionAction(HttpServletRequest req, HttpServletResponse resp)
		...
		if (decision.getTransStatus() == TransactionStatus.APPROVED) {
			try {			
								
				clientTransactionDAO.Transfer(transId);
			} catch (ServiceException e) {
